...
class CustomerRouting {
    ...
    static async checkOut(req, res, idCustomer, idOrder) {
        if (req.method === 'POST') {
            ...
            let house = await HouseService.findByIdHouse(order[0].idHous);
            let count = (+house[0].countCustomer) + 1;
            let mess = await HouseService.changeStatus(idCustomer, idOrder);
    ...
    static async cancelRent(req, res, idCustomer, idOrder) {
        if (req.method === 'POST') {
            let mess = await CustomerService.cancel(idCustomer, idOrder);
            res.writeHead(301, {'location': `/customer/myHouse/${idCustomer}`});
            res.end();
        }
    }
