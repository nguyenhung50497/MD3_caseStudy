        ...
        if (req.method === 'GET') {
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    let arrCheckIn = rentDetail.checkOut.split('-');
                    let arrCheckOut = rentDetail.checkOut.split('-');
                    // console.log(rentDetail.checkOut);
                    let house = await HouseService.findByIdHouse(idHouse);
                    let price = house[0].pricePerDay;
                    let totalMoney;
                    console.log(+arrCheckOut[2]);
                    if (arrCheckOut[0] === arrCheckIn[0]) {
                        if (arrCheckOut[1] === arrCheckIn[1]) {
                            totalMoney = price * ((+arrCheckOut[2]) - (+arrCheckIn[2]));
                        }
                    }

                    console.log(totalMoney);
                    // let mess = await CustomerService.rent(rentDetail, idHouse, idCustomer);
                    res.writeHead(200, 'text/html');
                    res.end();
                }
            })
