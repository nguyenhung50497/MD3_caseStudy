...
class HouseRouting {
        ...
        if (req.method === 'GET') {
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    let data = await HouseService.save(house);
                    console.log(data)
    ...
    static showFormUpLoad(req, res, id) {
        if (req.method === 'GET') {
            fs.readFile('./views/uploadImgHouse.html', 'utf-8', (err, upLoadHtml) => {
                if (err) {
                    console.log(err)
                } else {
                    res.writeHead(200, 'text/html');
    ...
    static editHouse(req, res, id) {
        if (req.method === 'GET') {
            fs.readFile('./views/editHouse.html', 'utf-8', async (err, editHtml) => {
                if (err) {
                    console.log(err);
                }
                else {
                    let house = await HouseService.findById(id);
                    ...
                    editHtml = editHtml.replace('{amountBathroom}', house[0].amountBedroom);
                    editHtml = editHtml.replace('{nameHouse}', house[0].nameHouse);
                    editHtml = editHtml.replace('{pricePerDay}', house[0].price);
    ...
    static async deleteHouse(req, res, id) {
        if (req.method === 'POST') {
            let mess = await HouseService.remove(id);
            res.writeHead(301, {'location': '/home'});
            res.end();
        }
    }
