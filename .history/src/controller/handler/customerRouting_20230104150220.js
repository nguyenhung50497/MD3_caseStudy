            ...
            fs.readFile('./views/customer/homeCustomer.html', 'utf-8', async (err, myHouseHtml) => {
                if (err) {
                    ...
                    myhouses.map((myhouse, index) => {
                        let checkIn = myhouse.checkIn;
                        let checkOut = myhouse.checkOut;
                        if (myhouse.statusOrder === 'true') {
                            tbody += `<tr>
                                    ...
                                    <td>${checkIn.toLocalDateString()}</td>
                                    <td>${checkOut.toLocalDateString()}</td>
                                    <td>${myhouse.timeRent}</td>
                                    <td>${myhouse.totalMoney}</td>
                                    <td><p>Đã từng thuê</p></td>
                                    <td>
                                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#checkOutModal${myhouse.idOrder}">Rent again</button>
                        ...
                        tbody += `<tr>
                                    ...
                                    <td>${myhouse.statusHouse}</td>
                                    <td>${myhouse.countCustomer}</td>
                                    <td>${myhouse.checkIn}</td>
                                    <td>${myhouse.checkOut}</td>
                                    <td>${myhouse.timeRent}</td>
                                    <td>${myhouse.totalMoney}</td>
                                    <td>
                                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cancelModal${myhouse.idOrder}">Cancel</button>
                                        <div class="modal fade" id="cancelModal${myhouse.idOrder}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
