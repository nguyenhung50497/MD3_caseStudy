        ...
        houses.map((house, index) => {
            if (house.statusHouse === "Đã thuê") {
                ...
                tbody += `
                    ...
                    <td>${house.statusHouse}</td>
                    <td>${house.countCustomer}</td>
                    <td><a href="/house/rent/${house.idHouse}/${idCustomer}"><button class="btn btn-primary">Rent</button></a></td>
                    </tr>
                    `
            }
        })
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    if (data.userName === "admin" && data.password === "admin") {
                        res.writeHead(301, {'location': `/customer/homeAdmin/${customer[0].idCustomer}`});
                        res.end();
                    }
                    else {
