...
class CustomerService {
    ...
    static changeStatusOrder(idOrder) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`UPDATE ordermanager SET statusHouse = 'Còn trống', countCustomer = ${+count} WHERE idHouse = ${idHouse}`, (err, status) => {
                if (err) {
                    reject(err);
                }
                else {
