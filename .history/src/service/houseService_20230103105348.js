...
class HouseService {
    ...
    static update(house, id) {
        return new Promise((resolve, reject) => {
            HouseService.connect.query(`UPDATE house SET nameHouse = '${house.nameHouse}', typeRoom = '${house.typeRoom}', addressHouse = '${house.addressHouse}', amountBedroom = ${house.am}, price = ${product.price}, description = '${product.description}', idCategory = '${product.idCategory}' WHERE id = ${id}`, (err, products) => {
                 if (err) {
                     reject(err);
                 }
                 else {
