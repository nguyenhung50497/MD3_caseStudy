...
class CustomerService {
    ...
    static updateCustomer(customer, id) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`UPDATE customer SET nameCustomer = '${customer.nameCustomer}', yearOfBirth = ${customer.yearOfBirth}, addressCustomer = '${customer.addressCustomer}', phoneNumber = ${customer.phoneNumber}, email = '${customer.email}', userName = '${customer.userName}', password = '${customer.password}' WHERE idHouse = ${id}`, (err, customers) => {
                 if (err) {
                     reject(err);
                 }
                 else {
