...
class HouseService {
    ...
    static save(house) {
        return new Promise((resolve, reject) => {
            HouseService.connect.query(`INSERT INTO my_database1.product(name, price, description, image, idCategory) VALUES ('${product.name}', ${product.price}, '${product.description}', '${product.image}', '${+product.idCategory}')`, (err, data) => {
                 if (err) {
                     reject(err);
                 }
                 else {
