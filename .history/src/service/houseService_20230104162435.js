...
class HouseService {
    ...
    static history() {
        return new Promise((resolve, reject) => {
            HouseService.connect.query('SELECT * FROM house JOIN ordermanager ON house.idHouse = ordermanager.idHouse JOIN customer ON ordermanager.idCustomer = customer.idCustomer ', (err, houses) => {
                if (err) {
                    reject(err);
                }
                else {
