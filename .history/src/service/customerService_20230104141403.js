...
class CustomerService {
    ...
    static rent(rent, idHouse, idCustomer, totalMoney) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`INSERT INTO ordermanager(checkIn, checkOut, totalMoney, idHouse, idCustomer, timeRent, statusOrder) VALUES ('${rent.checkIn}', '${rent.checkOut}', ${totalMoney}, ${idHouse}, ${idCustomer}, ${rent.timeRent}, 'fal')`, (err, data) => {
                if (err) {
                    reject(err)
                }
                else {
