...
class HouseService {
    ...
    static update(house, id) {
        return new Promise((resolve, reject) => {
            HouseService.connect.query(`UPDATE house SET nameHouse = '${house.nameHouse}', typeRoom = '${house.typeRoom}', addressHouse = '${house.addressHouse}', amountBedroom = ${house.amountBedroom}, amountBathroom = ${house.amountBathroom}, description = '${house.description}', pricePerDay = ${house.pricePerDay}, statusHouse = '${house.statusHouse}' WHERE idHouse = ${id}`, (err, house) => {
                 if (err) {
                     reject(err);
                 }
                 else {
