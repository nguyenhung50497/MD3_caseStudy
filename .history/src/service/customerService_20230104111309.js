...
class CustomerService {
    ...
    static rent(rent, idHouse, idCustomer, totalMoney) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`INSERT INTO ordermanager(checkIn, checkOut, idHouse, totalMoney, idCustomer, timeRent) VALUES ('${rent.checkIn}', '${rent.checkOut}', ${totalMoney}, ${idHouse}, ${idCustomer}, ${rent.timeRent})`, (err, data) => {
                if (err) {
                    reject(err)
                }
                else {
    ...
    static findMyHouseById(idCustomer) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`SELECT * FROM house JOIN ordermanager ON house.idHouse = ordermanager.idHouse WHERE idCustomer = ${id}`, (err, house) => {
                if (err) {
                    reject(err)
                }
                else {
