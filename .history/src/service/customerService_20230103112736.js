...
class CustomerService {
    ...
    static updateCustomer(house, id) {
        return new Promise((resolve, reject) => {
            CustomerService.connect.query(`UPDATE customer SET nameCustomer = '${house.nameHouse}', yearOfBirth = '${house.typeRoom}', addressCustomer = '${house.addressHouse}', phoneNumber = ${house.amountBedroom}, email = ${house.amountBathroom}, userName = '${house.description}', pricePerDay = ${house.pricePerDay}, statusHouse = '${house.statusHouse}' WHERE idHouse = ${id}`, (err, customers) => {
                 if (err) {
                     reject(err);
                 }
                 else {
